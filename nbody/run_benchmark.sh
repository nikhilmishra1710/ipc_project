...
for N in 10000 30000 50000; do
    ...
    echo -n "  Hybrid 2R×2T...    "; O=$(OMP_NUM_THREADS=2 mpirun --allow-run-as-root --oversubscribe -n 2 ./build/hybrid_nbody $N 2>&1)
    T_HYB22=$(gt "$O" TIME); echo "${T_HYB22}s"

    echo -n "  Hybrid 2R×4T...    "; O=$(OMP_NUM_THREADS=4 mpirun --allow-run-as-root --oversubscribe -n 2 ./build/hybrid_nbody $N 2>&1)
    T_HYB24=$(gt "$O" TIME); echo "${T_HYB24}s"

    echo -n "  Hybrid 4R×2T...    "; O=$(OMP_NUM_THREADS=2 mpirun --allow-run-as-root --oversubscribe -n 4 ./build/hybrid_nbody $N 2>&1)
    ...
    printf "  %-18s  %-10s  %-8s\n" "Hybrid 2R×2T"   "$T_HYB22" "${S_HYB22}x"
    printf "  %-18s  %-10s  %-8s\n" "Hybrid 2R×4T"   "$T_HYB24" "${S_HYB24}x"
    printf "  %-18s  %-10s  %-8s\n" "Hybrid 4R×2T"   "$T_HYB42" "${S_HYB42}x"
...
# ── Strong Scaling (fixed N=30000, vary worker count 1‥8) ─────────────────────
header "Strong Scaling  (N=30000, Velocity Verlet)"
N_SS=30000
SS_JS=""
T1_OMP=""; T1_MPI=""
for k in 1 2 4 8; do
    echo -n "  OMP k=$k...  "
    O=$(OMP_NUM_THREADS=$k ./build/omp_nbody $N_SS 2>&1)
    T_O=$(gt "$O" TIME); echo -n "${T_O}s"
    [ -z "$T1_OMP" ] && T1_OMP=$T_O
    S_O=$(spd "$T1_OMP" "$T_O")

    echo -n "   MPI k=$k...  "
    O=$(mpirun --allow-run-as-root --oversubscribe -n $k ./build/mpi_nbody $N_SS 2>&1)
    T_M=$(gt "$O" TIME); echo "${T_M}s"
    [ -z "$T1_MPI" ] && T1_MPI=$T_M
    S_M=$(spd "$T1_MPI" "$T_M")

    SS_JS="${SS_JS}{k:${k},omp:${T_O},mpi:${T_M},s_omp:${S_O},s_mpi:${S_M}},"
done

# ── Weak Scaling (N = base_N × k, so each worker has constant load) ──────────
header "Weak Scaling  (base_N=5000 per worker)"
BASE_N=5000
WS_JS=""
T0_OMP=""; T0_MPI=""
for k in 1 2 4 8; do
    N_k=$((BASE_N * k))
    echo -n "  k=$k N=$N_k  OMP... "
    O=$(OMP_NUM_THREADS=$k ./build/omp_nbody $N_k 2>&1)
    T_O=$(gt "$O" TIME); echo -n "${T_O}s"
    [ -z "$T0_OMP" ] && T0_OMP=$T_O
    E_O=$(eff "$T0_OMP" "$T_O")

    echo -n "   MPI... "
    O=$(mpirun --allow-run-as-root --oversubscribe -n $k ./build/mpi_nbody $N_k 2>&1)
    T_M=$(gt "$O" TIME); echo "${T_M}s"
    [ -z "$T0_MPI" ] && T0_MPI=$T_M
    E_M=$(eff "$T0_MPI" "$T_M")

    WS_JS="${WS_JS}{k:${k},n:${N_k},omp:${T_O},mpi:${T_M},e_omp:${E_O},e_mpi:${E_M}},"
done
...
write_simple_json "$SS_JS"   "ui/strong_scaling.json"
write_simple_json "$WS_JS"   "ui/weak_scaling.json"
