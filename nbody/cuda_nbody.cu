 ...
 * ----------------------------------------------------------------------- */
__global__ void forces_kernel(const float *x, const float *y, const float *z,
                              const float *mass, float *ax, float *ay,
                              float *az, int n, int tile_sz) {
  ...
  if(i<n){ ax[i]=fx; ay[i]=fy; az[i]=fz; }
}

/* Position update (Verlet step 1) */
__global__ void verlet_pos_kernel(float *x, float *y, float *z, const float *vx,
...
__global__ void verlet_vel_kernel(float *vx, float *vy, float *vz,
                                  const float *ax_old, const float *ay_old,
                                  const float *az_old, const float *ax_new,
                                  const float *ay_new, const float *az_new,
                                  int n, float dt) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float hdt = 0.5f * dt;
...
static void run_cuda(float *hx, float *hy, float *hz, float *hvx, float *hvy,
                     float *hvz, float *hm, int n, int tile_sz) {
  ...
  /* Initial forces */
  forces_kernel<<<blk, tile_sz, shm>>>(dx, dy, dz, dm, dax, day, daz, n,
                                       tile_sz);
  CK(cudaGetLastError());
  ...
  for (int t = 0; t < TIMESTEPS; t++) {
    ...
    verlet_pos_kernel<<<blk, tile_sz>>>(dx, dy, dz, dvx, dvy, dvz, dax, day,
                                        daz, n, DT);
    CK(cudaGetLastError());
    CK(cudaDeviceSynchronize());

    /* Step 2: new forces */
    forces_kernel<<<blk, tile_sz, shm>>>(dx, dy, dz, dm, dnax, dnay, dnaz, n,
                                         tile_sz);
    CK(cudaGetLastError());
    CK(cudaDeviceSynchronize());

    /* Step 3: update velocities */
    verlet_vel_kernel<<<blk, tile_sz>>>(dvx, dvy, dvz, dax, day, daz, dnax,
                                        dnay, dnaz, n, DT);
    CK(cudaGetLastError());
    CK(cudaDeviceSynchronize());
...
static void run_cuda(float *x, float *y, float *z, float *vx, float *vy,
                     float *vz, float *m, int n, int tile_sz) {
  (void)tile_sz;
...
int main(int argc, char **argv) {
  ...
  int tile_sz = argc > 2 ? atoi(argv[2])
                         : 256; /* runtime tile size — like OMP_NUM_THREADS */
